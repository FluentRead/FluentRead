...
.fr-image-spinner {width:14px;height:14px;flex:0 0 auto;box-sizing:border-box;border:2px solid #f7c9d6;border-top-color:var(--fr-image-brand);border-radius:50%;animation:fr-image-spin .75s linear infinite;}
.fr-image-spinner[data-animated=false] {animation:none;}
.fr-image-actions {display:flex;align-items:center;flex-wrap:wrap;gap:4px;padding:3px;border:1px solid var(--fr-image-line);border-radius:12px;background:var(--fr-image-surface);box-shadow:0 3px 12px rgba(27,36,57,.12);}
.fr-image-controls button,.fr-image-feedback button {all:unset;display:inline-flex;align-items:center;justify-content:center;box-sizing:border-box;min-height:28px;padding:4px 10px;border-radius:8px;cursor:pointer;color:inherit;font:inherit;font-size:12px;font-weight:550;white-space:nowrap;}
...
.fr-image-feedback[data-phase=error] {width:300px;padding:16px;pointer-events:auto;overflow:auto;}
...
.fr-image-feedback button {min-height:32px;padding:6px 12px;}
...
@media (prefers-reduced-motion: reduce) {.fr-image-spinner {animation:none;}}
`;
...
export function createImageControls(actions: {onAction(): void; onPrepare(): void; onDismiss?(): void; onInspect?(): void; translate?(source: string): string}) {
    ...
    const refreshLanguage = () => {
        heading.textContent = localize('图片翻译');
        ...
        if (phase === 'loading') status.prepend(spinner);
    };
    let disposed = false;
